-- ============================================================================
-- CheckinBliss — UNBLOCK: live booking + Stripe payment test (minimal patch)
-- ============================================================================
-- The live DB is missing: branded_name rename, slug columns, the slug-resolving
-- book_stays(), correctly-rolled demo users, and any property rows. The previous
-- curated patch did not persist. This slim script applies ONLY what the booking
-- -> PaymentElement flow needs (buttons/dates/guest -> confirm -> Stripe charge
-- + deposit hold). Idempotent — safe to run twice.
-- HOW TO RUN: paste into https://supabase.com/dashboard/project/ejwurxnkcxpenmhbqwdi/sql/new
-- and click Run. Paste back the final "verification" table output.
-- ============================================================================
...
union all select 'branded_name column', count(*) from information_schema.columns
  where table_schema = 'public' and table_name = 'properties' and column_name = 'branded_name';
select id, role, email from profiles where email like '%@checkbliss.com' order by email;
