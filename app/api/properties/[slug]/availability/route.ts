...
export async function GET(
  ...
  if (property) {
    const [reservations, blocks] = await Promise.all([
      db.from("reservations").select("check_in, check_out").eq("property_id", property.id).neq("status", "cancelled"),
      db.from("availability_blocks").select("starts, ends").eq("property_id", property.id),
    ]);
    const unavailable = [
      ...(reservations.data ?? []).map((r: { check_in: string; check_out: string }) => ({ from: r.check_in, to: r.check_out })),
      ...(blocks.data ?? []).map((b: { starts: string; ends: string }) => ({ from: b.starts, to: b.ends })),
    ];
