...
| Payments             | Airwallex                         | Hosted-fields card capture + manual-capture deposit holds     |
...
│       │   └── airwallex/route.ts          → PaymentIntent status events
│       └── cron/
...
│   ├── airwallex.ts                        → Charge + deposit hold, capture/release, mock mode
...
| `lib/`           | Domain logic + third-party clients (Supabase, Airwallex, WhatsApp) + currency + types.            |
...
book_stays() RPC — ONE transaction:
  ...
  (GiST EXCLUDE prevents overlap; any conflict → rollback, NO charge)
        ↓
Airwallex charge (auto-capture) + deposit hold (manual-capture)
        ↓
...
### Deposit hold lifecycle (Airwallex + cron)
...
| airwallex_authorisation_id | text     |                                             |
...
const charge = await airwallex.charge(group, { capture: "automatic",
  request_id: `charge-${group.id}` });             // idempotency key
const hold = await airwallex.hold(group, { capture: "manual",
...
**The server orchestrates; it owns none of the safety:** the database owns non-overlap (GiST), Airwallex owns money movement (manual-capture holds).
...
- **Card data never touches the server** — Airwallex hosted fields, no redirect; out of PCI-DSS scope (PRD 4.6).
...
- **Idempotency on payments** — Airwallex `request_id` keyed on `booking_group_id` prevents duplicate charges.
...
- **Mock mode must always work** — the full booking→payment→hold→confirmation flow runs with no Supabase/Airwallex/WhatsApp credentials.