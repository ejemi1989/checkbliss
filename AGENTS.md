...
- **Payments:** Airwallex (hosted fields + manual-capture deposit holds). **Not** Stripe.
...
- **Card data never touches the server.** Airwallex hosted fields only, no redirect. Never accept raw PAN/CVV.
...
- **Idempotency on payments** — Airwallex `request_id` keyed on `booking_group_id`.
- **Reviews are Trustpilot-only.** Never build on-platform reviews/ratings.
- **Mock mode must always work** — the full booking -> payment -> hold -> confirmation flow runs with no Supabase/Airwallex/WhatsApp credentials. Never introduce a hard dependency that breaks mock mode.
...
- `lib/` — domain logic + third-party clients (`supabase.ts`, `airwallex.ts`, `whatsapp.ts`), `currency.ts`, `types.ts`, `data.ts`.
...
`POST /api/bookings`: Zod + 14-day check -> `book_stays()` atomic RPC (overlap -> rollback, no charge) -> Airwallex charge (auto) + deposit hold (manual) -> `confirm_booking_group()` -> persist `deposit_holds` (7-day expiry) -> owner WhatsApp -> audit. Payment failure after reserve -> compensating group cancel (dates freed, nothing charged). **The server orchestrates; the database owns non-overlap and Airwallex owns money movement.**

## Environment

All credentials in `.env.example`; the app runs in mock mode with none set. Keys: Supabase (URL + publishable + secret), Airwallex (`AIRWALLEX_ENV`, client id, api key), WhatsApp (`WHATSAPP_VERIFY_TOKEN`, `WHATSAPP_APP_SECRET`, phone number id, access token), `CRON_SECRET`, `ADMIN_DASH_KEY`. Never hardcode or commit keys.
...
- **Airwallex call failing** — mock mode triggers when `AIRWALLEX_CLIENT_ID` is unset; if it's set but wrong, mock mode won't engage and real API will reject. Unset the env var to force mock mode.
